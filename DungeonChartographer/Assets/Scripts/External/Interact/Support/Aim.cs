...
public class Aim : MonoBehaviour, ITFuncStr
{
        ...
        if(aim){
            ...
            if(autoTrack){
                ...
                if(trig == 0){
                    ...
                    targetDir *= UnityEngine.Random.Range(1, 10);
                    targetDir += UnityEngine.Random.insideUnitCircle * rngWeight;
                    if(sum < close/2f){
                        trig += 1;
                        closeOff += UnityEngine.Random.Range(-close/2f, close/2f);
                    }
                }else if(trig == 2){
                    rSpeed *= damp;
                    if(sum > closeOff){
...
public interface ITFuncStr{
    MonoBehaviour Obj { get; }
    void Func(List<string> args, List<object> oargs);
}